...
}); // Removes the word 'literally'

storyWords = storyWords.map((word) => {
  return word === misspelledWord ? 'beautiful' : word; 
}); //Replaces 'beautifull' with 'beautiful'

let badWordIndex = storyWords.findIndex((word) => {
 return word === badWord;
});
console.log(badWordIndex); // Prints out the index of the word 'freaking'

storyWords[78] = 'really'; // replaces the word in index position 78 with 'really'

let lengthCheck = storyWords.every((word) => {
  return word.length <= 10;
});
console.log(lengthCheck); // If 'False' there are words with more than 10 characters

storyWords.forEach((word) => {
  word.length > 10 && console.log(word); // Prints out the word with more tha 10 characters
});
